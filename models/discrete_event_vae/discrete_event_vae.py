...
class dVAELogit(nn.Module):

    def __init__(self, nevents_per_token=64, dim_embedding=256, vocab_size=1024):
        super(dVAELogit, self).__init__()
    ...
    def forward(self, x):
        ...
        with torch.no_grad():
            logits = self.dvae(x)
        target = torch.argmax(logits, dim=-1)
        target = target.view(batch_size, -1)  # (batch_size, seq_len)
        # to 1-hot
        target = torch.nn.functional.one_hot(target, self.vocab_size).float()  # (batch_size, seq_len, vocab_size)
        output = target.sum(dim=1)  # (batch_size, vocab_size)
        return output
